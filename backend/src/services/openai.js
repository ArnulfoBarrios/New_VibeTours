...
export async function generateChatResponse(state, backendInstruction = '', webSearchSummary = '', currentPreferences = {}) {
  ...
  if (!apiKey) {
    ...
    if (!hasCity) {
      ...
      const isExplicitBuildRequestedByUser = /\b(gener(ar|es|a|e|en|al)?\s+(el\s+|la\s+)?(tour|itinerario|ruta|viaje|plan|mapa)|cre(ar|es|a|e|en)?\s+(el\s+|la\s+)?(tour|itinerario|ruta|viaje|plan|mapa)|adelante(\s+(general|genera|crea|construye))?|inicia(r)?\s+(el\s+|la\s+)?(tour|itinerario|ruta)|finaliza(r)?\s+(el\s+|la\s+)?(tour|itinerario|ruta)|constru(ye|ir)\s+(el\s+|la\s+)?(tour|itinerario|ruta|viaje)|dise[ñn](ar|a|es|e)?\s+(el\s+|la\s+)?(tour|itinerario|ruta)|est[aá]\s+perfecto\s+(genera|crea)|listo\s+(genera|crea)|ya\s+no\s+hay\s+nada\s+genera|listo\s+para\s+generar|vale\s+genera|procede\s+a\s+generar|si\s+genera\s+(el\s+|la\s+)?(tour|itinerario|ruta)|s[íi]\s+genera\s+(el\s+|la\s+)?(tour|itinerario|ruta)|genera\s+(el\s+|la\s+)?(tour|itinerario|ruta)\s+porfa|crea\s+(el\s+|la\s+)?(tour|itinerario|ruta)|haz\s+(el\s+|la\s+)?(tour|itinerario|ruta)|quiero\s+(que\s+)?(se\s+)?gener(ar|es|a|e)?|ok\s+quiero\s+generar|adelante\s+crea|adelante\s+procede|vamos\s+genera|armar\s+(el\s+|la\s+)?(tour|itinerario|ruta|viaje))\b/i.test(lastUserMsg)
