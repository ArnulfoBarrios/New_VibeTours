...
test('5 reported screenshot places resolve with meter-level precision', async () => {
  ...
  // 3. Restaurante Cucayo (Calle 85 # 52-153)
  const cucayo = await geocodePlace('Restaurante Cucayo', bqLat, bqLon, regionalOpts)
  assert.ok(cucayo, 'Cucayo must resolve')
  const distCucayo = haversineMeters(cucayo.latitude, cucayo.longitude, 11.0074, -74.8174)
  assert.ok(distCucayo < 100, `Cucayo is ${distCucayo}m from Calle 85 # 52-153 (must be < 100m)`)
