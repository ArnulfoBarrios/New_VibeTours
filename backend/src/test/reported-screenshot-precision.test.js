...
test('5 reported screenshot places resolve with meter-level precision', async () => {
  ...
  const distNarcobollo = haversineMeters(narcobollo.latitude, narcobollo.longitude, 11.0053, -74.8213)
  assert.ok(distNarcobollo < 150, `Narcobollo is ${distNarcobollo}m from Cra 43 # 84-188 (must be < 150m)`)

  // 3. Restaurante Cucayo (Cra 49C # 76-08)
  const cucayo = await geocodePlace('Restaurante Cucayo', bqLat, bqLon, regionalOpts)
  assert.ok(cucayo, 'Cucayo must resolve')
  const distCucayo = haversineMeters(cucayo.latitude, cucayo.longitude, 10.9972, -74.8095)
  assert.ok(distCucayo < 150, `Cucayo is ${distCucayo}m from Cra 49C # 76-08 (must be < 150m)`)
