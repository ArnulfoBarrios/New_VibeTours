...
test('5 reported screenshot places resolve with meter-level precision', async () => {
  ...
  // 3. Restaurante Cucayo (Cra 53 # 85-33)
  const cucayo = await geocodePlace('Restaurante Cucayo', bqLat, bqLon, regionalOpts)
  assert.ok(cucayo, 'Cucayo must resolve')
  const distCucayo = haversineMeters(cucayo.latitude, cucayo.longitude, 11.0118, -74.8213)
  assert.ok(distCucayo < 100, `Cucayo is ${distCucayo}m from Cra 53 # 85-33 (must be < 100m)`)
