...
describe('Tour Planner AI Behavior and Filtering Rules', () => {
  ...
  it('should validate places strictly with isValidSpecificPlace discarding days, times, amenities and bare cities', async () => {
    const { isValidSpecificPlace } = await import('../routes/ai.js')
    
    // Invalid elements (must return false)
    assert.equal(isValidSpecificPlace('Día 1'), false)
    assert.equal(isValidSpecificPlace('Dia 2: Historia'), false)
    assert.equal(isValidSpecificPlace('Day 3'), false)
    assert.equal(isValidSpecificPlace('Mañana'), false)
    assert.equal(isValidSpecificPlace('Tarde'), false)
    assert.equal(isValidSpecificPlace('Noche'), false)
    assert.equal(isValidSpecificPlace('Almuerzo'), false)
    assert.equal(isValidSpecificPlace('Cena'), false)
    assert.equal(isValidSpecificPlace('Atardecer'), false)
    assert.equal(isValidSpecificPlace('Alojamiento'), false)
    assert.equal(isValidSpecificPlace('Punto de partida'), false)
    assert.equal(isValidSpecificPlace('Descanso'), false)
    assert.equal(isValidSpecificPlace('Paseo'), false)
    assert.equal(isValidSpecificPlace('Bailar'), false)
    assert.equal(isValidSpecificPlace('Hotel Casa La Fe'), false)
    assert.equal(isValidSpecificPlace('Cartagena'), false)
    assert.equal(isValidSpecificPlace('Medellín'), false)
    assert.equal(isValidSpecificPlace('Bogotá'), false)

    // Valid physical POIs (must return true)
    assert.equal(isValidSpecificPlace('Castillo San Felipe de Barajas'), true)
    assert.equal(isValidSpecificPlace('Muelle de La Bodeguita'), true)
    assert.equal(isValidSpecificPlace('Plaza Santo Domingo'), true)
